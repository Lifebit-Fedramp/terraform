...
resource "aws_security_group_rule" "allow_cidr" {
  for_each          = toset(var.source_cidr_blocks)
...
resource "aws_security_group_rule" "allow_sgs" {
  for_each                 = toset(var.source_sgs)
  security_group_id        = aws_security_group.default.id
  type                     = "ingress"
  from_port                = var.port
  to_port                  = var.port
...
resource "aws_secretsmanager_secret_version" "this" {
  ...
  secret_string = jsonencode({
    ...
    REDIS_PORT : aws_elasticache_replication_group.default.port
  })
}
