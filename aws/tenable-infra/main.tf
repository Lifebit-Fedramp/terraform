...
locals {
  user_data = <<-EOT
    ...
    TENABLE_WAS_NAME=$(aws ssm get-parameter --region us-gov-west-1 --name /TENABLE_WAS_NAME --with-decryption | jq -r '.Parameter.Value')
    ...
    docker run -d -e WAS_SCANNER_NAME=$TENABLE_WAS_NAME -e WAS_LINKING_KEY=$NESSUS_KEY tenable-was::latest 
  EOT
}
