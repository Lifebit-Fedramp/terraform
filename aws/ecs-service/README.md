...
This module requires an existing ECS Cluster and ALB. The ALB Security group id and listener arn are passed in. ALB Target 
groups and listener rules are created in this module, as well as a security group allowing traffic from the ALB security group.

Secrets that are created (by passing in a list of secrets `secrets_list` in the container configuration) are created at the
path `"<terraform workspace>/<app name>/ecs-configuration"` with the keys provided in the list and empty values. These must
be manually populated through the console after creation.
...
| Name | Source | Version |
|------|--------|---------|
| <a name="module_ecs_service"></a> [ecs\_service](#module\_ecs\_service) | terraform-aws-modules/ecs/aws//modules/service | 5.11.1 |

## Resources

| Name | Type |
|------|------|
| [aws_lb_listener_rule.service_rule](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/lb_listener_rule) | resource |
...
| <a name="input_alb_sg_id"></a> [alb\_sg\_id](#input\_alb\_sg\_id) | SG ID of ALB associated with service | `string` | `""` | no |
| <a name="input_app_port"></a> [app\_port](#input\_app\_port) | App port for target group to listen on | `number` | `80` | no |
...
| <a name="input_create"></a> [create](#input\_create) | Determines whether service resources will be created | `bool` | `true` | no |
...
| <a name="input_health_check_path"></a> [health\_check\_path](#input\_health\_check\_path) | Health check path for the service target group | `string` | `"/"` | no |
...
| <a name="input_listener_arn"></a> [listener\_arn](#input\_listener\_arn) | Listener ARN to add service listener rule to | `string` | `""` | no |
...
| <a name="input_path_pattern"></a> [path\_pattern](#input\_path\_pattern) | Path pattern to match on for service listener rule | `list(string)` | <pre>[<br>  "/*"<br>]</pre> | no |
...
| <a name="input_rule_priority"></a> [rule\_priority](#input\_rule\_priority) | Priority for service listener rule | `number` | `100` | no |
...
| <a name="input_target_type"></a> [target\_type](#input\_target\_type) | Target type for the service target group | `string` | `"ip"` | no |
...
| <a name="input_tg_protocol"></a> [tg\_protocol](#input\_tg\_protocol) | Protocol for the service target group | `string` | `"HTTP"` | no |
...
| <a name="output_service_secrets"></a> [service\_secrets](#output\_service\_secrets) | ARN of the secrets created for the services |
